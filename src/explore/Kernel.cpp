...
void CKernel::setMaxValueToMask(double maxValue)
{
    // find current max value
    double curMax = -1.0;
    for(int s = 0; s < m_maskLength; s++)
    {
        if(m_kernelMask[s]>curMax)
            curMax = m_kernelMask[s];
    }

    double scale = maxValue/curMax;
    // update values of all cells inside the mask
    for(int s = 0; s < m_maskLength; s++)
    {
        m_kernelMask[s] *= scale;
    }
}

void CKernel::print(void)
{
    ...
    for(int y=0; y < m_maskSizeY; y++)
    {
        ...
        {
            printf("%.3f ",m_kernelMask[x+m_maskSizeX*y]);
        }
        printf("\n");
    }
}
