...
#            print count
			...
			if aux == False:
				...
				pts[i].append([float(aux), float(s)])
    return pts
			...
			for p in g:
				if p.tag == prefix+'g':
					for m in p:
						if m.tag == prefix+'g':
							for n in m:
								if n.tag == prefix+'path':
									points = getPointsUpdated(n.attrib.get('d'))
									print points
									ntx,nty = getTranslation(n.attrib.get('transform'))
